<template>
    <div class="drawer-list scrollbar" @scroll="listScroll">
        <div class="item draggable flex align-center" v-for="(item, index) in list" :key="index" :id="index"
            ...
            <a-switch :checked="item.status == 'enable' ? true : false" checked-children="开" un-checked-children="关"
                @change="helloChange($event, item)" />
        </div>
    </div>

    <!-- 新建 -->
    <a-modal v-model:visible="addVisible" :centered="true" :mask="false" :width="600" title="新建打招呼回复" @ok="handleOk"
        @cancel="addClose">
...
export default {
    ...
    setup(props, contact) {
        const state = reactive({
            ...
            list: [],
            listForm: { page: 1, offset: 20, type: "hello" }, //分页
            page: 0, //总页数
            form: {}, //新建表单
            detailForm: {}, //修改表单
