<template>
    <div class="drawer-list scrollbar" @scroll="listScroll">
        <div class="item draggable flex align-center" v-for="(item, index) in list" :key="index" :id="index"
            ...
            <a-button type="dashed" size="small" @click="sendHland(item)" class="ml10 bc-them t-them" v-show="userId"> 发送</a-button>
        </div>
    </div>

    <!-- 新建 -->
    <a-modal v-model:visible="addVisible" :centered="true" :mask="false" :width="600" title="新建快捷回复" @ok="handleOk"
        @cancel="addClose">
...
export default {
    ...
    props: ['addVisible', 'userId'],
    emits: ['addClose', 'sendQuick'],
    setup(props, contact) {
        const state = reactive({
            menu: { top: '', left: '', visible: '', id: '', index: '' }, //右键菜单
            list: [],
            listForm: { page: 1, offset: 20, type: "quick_reply" }, //分页
            page: 0, //总页数
            form: {}, //新建表单
            detailForm: {}, //修改表单
