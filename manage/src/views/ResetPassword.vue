            ...
            <div class="w50 pl100 pr50 tel-layout tel-pd10">
                ...
                <a-form :model="form" autocomplete="off" @finish="onFinish" :wrapper-col="{ span: 20 }">
                    ...
                    <a-form-item name="new_password"
                        :rules="[{ required: true, pattern: /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[\da-zA-Z]{6,14}$/, message: '密码6至14位包含大写和小写字母' }]">
                        <a-input-password v-model:value="form.new_password" autocomplete="off" :bordered="false"
                            placeholder="请输入新密码" class="input" />
                    </a-form-item>
                    <a-form-item name="two_new_password" :rules="[{ required: true, validator: validatepasssure }]">
                        <a-input-password v-model:value="form.two_new_password" autocomplete="off" :bordered="false"
                            placeholder="请确认新密码" class="input" />
                    </a-form-item>
                    <a-form-item>
