...
const sendMsg = new Audio(import.meta.env.VITE_BASE_URL + '/common/audio/sendMsg.mp3');
/**
 * 全局方法
 * @localGet  ---获取缓存
 * @localSet  ---存入缓存
...
export function getElementData(id, type) {
    ...
    else return document.getElementById(id).clientWidth    
}
/**
 * 消息音频播放
 * @returns 
...
export function playMp3() {    
    if (typeof WeixinJSBridge == "undefined") return sendMsg.play();  // 播放
    WeixinJSBridge.invoke('getNetworkType', {}, function (e) {        
        return sendMsg.play();  // 播放
    }, false);
}
/**
 * 上传文件前
...
export async function uploadFile(file,name="") {
    return new Promise(async (resolve, reject) => {
        //获取上传签名
        const res = await axios.post("/common/api/oss_config", { file_name: file.name || 'file.'+name }, false);        
